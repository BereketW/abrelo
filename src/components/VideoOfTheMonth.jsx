...
export default function VideoOfTheMonth() {
  ...
  useEffect(() => {
    async function getData() {
      try {
        ...
        const videoDetailsPromises = currentMonthVideos.map((video) =>
        ...
        const videoOfTheMonth = videosWithStats.reduce(
          (maxVideo, currentVideo) => {
            return currentVideo.views > maxVideo.views
              ? currentVideo
              : maxVideo;
          }
        );
  ...
  return (
    <div
      className={`${
        pathurl.includes("admin") || pathname === hidden ? "hidden" : ""
      }  relative isolate dark:bg-[#262d34] -mb-20`}
    >
