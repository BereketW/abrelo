...
export default function Header({className=''}) {
  ...
  return (
    <header id="home" className={`${pathurls.includes("admin") ? "hidden":""}`}>
      ...
      <div className={`z-50 ${display ? "w-[400px] opacity-100" :" w-0 opacity-0 hidden"} hidden lg:block text-black  dark:border-none transition-all duration-200  dark:text-white dark:bg-channel-dark  p-4 flex flex-col w-[400px]   shadow-md bg-white  overflow-y-scroll absolute top-16 ${pathname === "/" ? "right-0" : "right-16 border border-gray-300 rounded"}`}>
